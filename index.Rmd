...
library(pacman)
p_load(shiny,flexdashboard, tidyverse,ggiraph, scales, sf, reshape2, cowplot, data.table, classInt,rmapshaper)
```
...
county <- st_read("shapefiles/County.shp") |> ms_simplify(keep = 0.001)
...
background_characteristics_map <- rbind(women_background_xtics_county, men_background_xtics_county)
...
background_characteristics_map <- full_join(county_updated, background_characteristics_map, by="County")|>
...
renderGirafe({
  ...
  marriage_data_map<- ggplot(marriage_data1 |>filter(type%in%input$marriage) |> filter(!(category%in%c("Women (20-49 yrs)", "Men (25-54 yrs)"))), 
                             aes(fill=cat)) + 
...
# You are here 
median_age_county <- fread("data/table_5/Table 6c. Median.age.county.first.birth.csv")
# |>
#   select(-V1) |>
...
renderPlot({
  ...
  bindCache(input$fertility_rates)
...
fp_messages <-fread("data/table_7/table_7_20_fp_messages_total.csv") |>
  mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
...
fp_messages_county <-fread("data/table_7/table_7_20C_fp_messages_total_county.csv")
# |>
#   mutate(County=recode(County,"Elgeyo/Marakwet"="Elgeyo Marakwet", "Murang’a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi", "Nairobi City"="Nairobi", "Taita/Taveta"="Taita Taveta"))|>
...
```{r, include=FALSE}
early_childhood_mortality_county <-fread("data/DHS County tables/Table_8.3C_grp_County Early Childhood Mortality.csv")
# |>
#   mutate(county=recode(county,"Elgeyo/Marakwet"="Elgeyo Marakwet", "Murang’a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi"))
...
},width = 1200, height=650) %>% bindCache(input$early_childhood_mortality_county2)
...
```{r, include=FALSE}
perinatal_mortality_county <-fread("data/DHS County tables/Table_8.4C_grp_Perinatal Mortality By County.csv")
# |>
#   mutate(county=recode(county,"Elgeyo/Marakwet"="Elgeyo Marakwet", "Murang’a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi"))|>
#   filter(category%in%"Perinatal mortality rate")
...
antenatal_care1 <-fread("data/table_9/table_9_2_ancvisits_number_by_background_xtics.csv")
# |>
#    mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
...
anc_number_county <-fread("data/table_9/table_9_2C_ancvisits_number_by_county.csv")
# |>
#   mutate(County=recode(County,"Elgeyo-Marakwet"="Elgeyo Marakwet", "Murang'a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi", "Nairobi City"="Nairobi", "Taita/Taveta"="Taita Taveta"))|>
...
renderPlot({ggplot(place_delivery[place_delivery$Variable%in%input$place_delivery,],aes(x=Characteristic, y=proportion, fill=category))+
  ...
  theme(text=element_text(size=16, face="bold"))}, width=1400, height=650 ) %>% bindCache(input$place_delivery)
...
renderPlot({ggplot(mens_involvement[mens_involvement$Variable%in%input$mens_involvement,],aes(x=category, y=proportion, fill=Characteristic))+
  ...
  bindCache(input$mens_involvement)
...
renderPlot({
  ...
  coord_flip()+
    scale_fill_brewer(palette = "Set1")
}, width=1200, height=800) %>% bindCache(input$trtsrc)
```
...
```{r, include=F}
nutrition_women_county <- fread("data/DHS County tables/table11_13C_grp_Nutrition_women_by_county.csv")
# |>
#   mutate(County=recode(County,"Elgeyo-Marakwet"="Elgeyo Marakwet", "Murang'a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi"))
...
```{r, include=F}
iron_women_county <- fread("data/DHS County tables/table9_4C_Iron_grp_supplement_women_by_county.csv")
# |>
#   mutate(County=recode(County,"Elgeyo-Marakwet"="Elgeyo Marakwet", "Murang'a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi"))
 ...
 arth1x <- girafe(ggobj = arth1,
  ...
  x <- girafe(ggobj = map1,
                   options = list(
                     ...
                     opts_selection(css = "fill:orange;", type = "multiple"),
                     opts_zoom(max = 4)), width=15, height=5)
  print(x)}}) %>% bindCache(input$arthritis)
...
renderPlot({ggplot(house_land_ownsership[house_land_ownsership$`Background characteristics`%in%input$home_own,],aes(y = Category, x=value, fill = variable)) + 
  ...
  theme(text = element_text(size=16, face="bold"))}, width=1500, height=800)  %>% bindCache(input$home_own)
...
renderPlot({ggplot(pers_physical_violence[pers_physical_violence$Gender%in%input$viol,],aes(y=value, x=reorder(Category,value), fill=variable)) +ylim(0,100)+coord_flip()+
  ...
  theme_bw() +scale_fill_brewer(palette = "Set1")+
  labs( x = "", y="%", fill="")+theme(text=element_text(size=16, face="bold"))
  }, width=1500, height=700) %>% bindCache(input$viol)
...
renderPlot({ggplot(effect_female_circumcision[effect_female_circumcision$Characteristic%in%input$effect_female_circumcision,],aes(y=value, x=reorder(variable,value), fill=Category)) +coord_flip()+
  geom_col(position="dodge") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(0.7), vjust=-0.5)+
  theme_bw() +scale_fill_brewer(palette = "Set1")+
  labs( x = "", y="%", fill="")+theme(text=element_text(size=20, face="bold"))}, width=1500, height=700)%>% bindCache(input$effect_female_circumcision)
...
renderPlot({ggplot(help_seeking_women[help_seeking_women$`Background characteristic`%in%input$help_seeking_women,],aes(y=value, x=Category, fill=variable)) +
  geom_col(position="dodge") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(0.7), vjust=-0.5)+
  theme_bw() +scale_fill_brewer(palette = "Set1")+
  labs( x = "", y="%", fill="")+theme(text=element_text(size=20, face="bold"))}, width=1500, height=700) %>% bindCache(input$help_seeking_women)
...
renderPlot({
  ...
  scale_fill_gradient2()+
    ...
    theme(text=element_text(size=14, face="bold"))
}, width=900, height=600)%>% bindCache(input$category1)
...
renderGirafe({
  ...
  comp_data_map<- ggplot(comparison_data2 |>filter(Category%in%input$ind_comp), aes(fill=Proportion)) + 
...
renderPlot({
  ...
  ggplot(comparison_data1 |> filter(Category%in%input$ind_comp, Prop%in%c("Proportion (2022)", "Proportion (2014)")), 
         aes(x=reorder(County, Proportion), y=Proportion, fill=factor(year))) + 
...
}, width = 900, height=700) %>% bindCache(input$ind_comp)
```
...
renderGirafe({
  ...
  combined_data1 <- left_join(combined_data |> filter(indicator1 %in% input$comp_indicator1) |> select(-indicator), 
                              combined_data |> filter(indicator1 %in% input$comp_indicator2) |> rename(indicator2 = indicator1, values2 = values) |>
                              select(-indicator)
                              )
